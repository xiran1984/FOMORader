...
- [ ] **项目初始化**
    - [x] `npm create vite@latest fomorader -- --template react-ts`
    - [x] 安装 JS 依赖：
        ```bash
    ...
    - [x] 安装 Python 依赖：
        ...
        ```
    - [x] 创建 `.ai/` 文件夹，放入项目文档（RULES / ARCHITECTURE / CHANGELOG 等）
    - [x] 创建 `.env`，写入：
        ```x
...
- [x] **数据获取 · RSS（国际 AI 一手信息，免费）**
    - [x] 脚本：`scripts/fetch_rss.py`
    - [x] 数据源（9个）：
    ...
    - [x] 运行并验证：
        ...
        ```
    - [x] 输出：`data/hotspots_raw.json`（标准结构，按 published_at 降序）

---

- [x] **数据获取 · X（中文高质量推文，twitterapi.io 付费）**
    - [x] 注册 [twitterapi.io](https://twitterapi.io)，充值（最低 $10，≈ 6.6万条额度，积分不过期）
    - [x] `.env` 写入 `TWITTERAPI_KEY`
    - [x] 模块文件职责（scripts/x-collector/）：
...
- [x] **数据处理 · Seed（JSON → SQLite）**
    - [x] 脚本：`scripts/seed.ts`
    - [x] 运行：`npx tsx scripts/seed.ts`
    - [x] 自动创建 `data/fomorader.db`，建立三张表：
        - `hotspots`：原始热点数据（由 seed.ts 写入）
        - `scores`：AI 评分结果（由阶段二 scorer.ts 写入）
        - `cache`：LLM 调用缓存，键为 `url + model_version + rules_version`
    - [x] 增量更新：`ON CONFLICT(url) DO UPDATE`，重复跑安全
    - [x] 支持 `--reset` 清空重建（慎用，会清空 scores）
    - [x] 验证：终端打印各来源条数统计
